export function classifySoilUSDA(
  sand: number,
  silt: number,
  clay: number,
): string {
  ...
  if (clayPct >= 40 && sandPct <= 45 && siltPct <= 40) return "Argiloso";
  if (clayPct >= 40 && siltPct >= 40) return "Argilo-silte";
  if (
    clayPct >= 27 &&
    clayPct < 40 &&
    sandPct >= 20 &&
  ...
  if (clayPct < 27 && sandPct >= 43 && sandPct <= 85) return "Franco-arenoso";

  if (clayPct < 20 && sandPct > 85 && siltPct < 15) return "Arenoso";
  if (clayPct < 12 && sandPct < 52 && siltPct > 50) return "Silte";
